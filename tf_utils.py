...
def max_pool(x, k_sz=[2, 2]):
    ...
    return tf.nn.max_pool(
        x, ksize=[1, k_sz[0], k_sz[1], 1],
        strides=[1, k_sz[0], k_sz[1], 1],
        padding='SAME')
...
def conv2d(x, n_kernel, k_sz, stride=1):
    ...
    W = tf.Variable(tf.random_normal(
        [k_sz[0], k_sz[1], int(x.get_shape()[3]), n_kernel]))
...
def fc(x, n_output, scope="fc", activation_fn=None, initializer=None):
    """fully connected layer with elu activation wrapper
    Args
    ...
    with tf.variable_scope(scope):
        if initializer is None:
            ...
            W = tf.Variable(tf.random_normal(
                [int(x.get_shape()[1]), n_output]), name='W')
            b = tf.Variable(tf.random_normal([n_output]), name='b')
        else:
            W = tf.get_variable(
                "W", shape=[int(x.get_shape()[1]), n_output], initializer=initializer)
