...
def skeleton(suffStat, indepTest, alpha, labels, m_max):
    ...
    Returns:
        sk+sepset: {"skeleton":np.array(G),"seperation set":d-seperation set for each point pair}
    """    
        ...
        for x, y in ind:
            if G[x][y] == True:
                ...
                neighbors = [i for i in range(
                    len(neighborsBool)) if neighborsBool[i] == True]
                ...
                if len(neighbors) >= ord:
                    ...
                    for neighbors_S in set(itertools.combinations(neighbors, ord)):
                        ...
                        pval,correlation_r = indepTest(suffStat, x, y, list(neighbors_S))
...
def extendCpdag(graph):
    ...
    pdag = [[0 if graph['sk'][i][j] == False else 1 for i in range(
...
if __name__ == '__main__':
    ...
    df_graph.to_csv('./a.csv',index=False)
